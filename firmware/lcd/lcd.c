...
uint8_t lcd_i2c_addr = (0x3F << 1);

HAL_StatusTypeDef LCD_SendInternal(uint8_t data, uint8_t flags) {
    HAL_StatusTypeDef res;
    for(;;) {
        res = HAL_I2C_IsDeviceReady(&I2C_HANDLE, lcd_i2c_addr, 3, HAL_MAX_DELAY);
        if(res == HAL_OK)
            break;
    }
...
void LCD_Init() {
    ...
    HAL_StatusTypeDef res;
    res = HAL_I2C_IsDeviceReady(&I2C_HANDLE, (0x27 << 1), 3, HAL_MAX_DELAY);
    if(res == HAL_OK) {
        lcd_i2c_addr = (0x27 << 1);
    }
